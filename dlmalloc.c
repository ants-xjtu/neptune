...
void InitHeap() {
  memset(heap_start, 0x0, sizeof(struct malloc_state));
  morecore_finished = 0;
  HeapFree(HeapMalloc(42));
}

#define get_malloc_state() ((struct malloc_state *)heap_start)

#include <stdint.h>

void *PrivateMoreCore(intptr_t inc) {
  if (morecore_finished) {
    if (inc == 0) {
      return heap_end;
    } else {
    ...
    return heap_start + sizeof(struct malloc_state);
  }
}
...
void *HeapRealloc(void *object, size_t size) {
  return dlrealloc(object, size);
}
