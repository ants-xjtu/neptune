                ...
                const u_char *packet)
{
    ...
    pi->packet = packet;
    pi->pheader = pheader;
    // olog("[*] Qcloud: we enter callback and finish initialization.\n");
    // fflush(stdout);
    set_pkt_end_ptr(pi);
...
//const struct in6_addr *ip_s)
{
    ...
    case AF_INET:
    {
        ...
        for (i = 0; i < MAX_NETS && i < nets; i++)
        {
            ...
            dlog("ip: %s\n", output);
#endif
            if ((IP4ADDR(ip) & IP4ADDR(&network[i].mask)) == (IP4ADDR(&network[i].addr) & IP4ADDR(&network[i].mask)))
            {
                our = 1;
                break;
            }
...
#else
                if ((ip_s.__u6_addr.__u6_addr32[0] & network[i].mask.__u6_addr.__u6_addr32[0]) == network[i].addr.__u6_addr.__u6_addr32[0] && (ip_s.__u6_addr.__u6_addr32[1] & network[i].mask.__u6_addr.__u6_addr32[1]) == network[i].addr.__u6_addr.__u6_addr32[1] && (ip_s.__u6_addr.__u6_addr32[2] & network[i].mask.__u6_addr.__u6_addr32[2]) == network[i].addr.__u6_addr.__u6_addr32[2] && (ip_s.__u6_addr.__u6_addr32[3] & network[i].mask.__u6_addr.__u6_addr32[3]) == network[i].addr.__u6_addr.__u6_addr32[3])
                {
                    our = 1;
                    break;
                }
...
void parse_tcp(packetinfo *pi)
{
    ...
    {
        if (IS_CSSET(&config, CS_TCP_CLIENT) && !ISSET_DONT_CHECK_CLIENT(pi))
        {
            if (pi->af == AF_INET)
    ...
    {
        if (IS_CSSET(&config, CS_TCP_SERVER) && !ISSET_DONT_CHECK_SERVICE(pi))
        {
            if (pi->af == AF_INET)
...
bastard_checks:
    if (IS_COSET(&config, CO_ACK) && TCP_ISFLAGSET(pi->tcph, (TF_ACK)) && !TCP_ISFLAGSET(pi->tcph, (TF_SYN)) && !TCP_ISFLAGSET(pi->tcph, (TF_RST)) && !TCP_ISFLAGSET(pi->tcph, (TF_FIN)))
    {
...
void parse_udp(packetinfo *pi)
{
    ...
    if (IS_CSSET(&config, CS_UDP_SERVICES))
    {
        if (pi->af == AF_INET)
        {
            ...
            // UPD Fingerprinting
            if (IS_COSET(&config, CO_UDP))
                fp_udp4(pi, pi->ip4, pi->udph, pi->end_ptr);
        }
        else if (pi->af == AF_INET6)
        {
            if (!ISSET_DONT_CHECK_SERVICE(pi) || !ISSET_DONT_CHECK_CLIENT(pi))
