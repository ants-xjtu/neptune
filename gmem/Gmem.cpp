...
namespace
{
	...
	public:
		// checkFunc从module中找_Gmem_
		Protection(Module *M)
		{
			this->M = M;
			this->inliningInfo = new InlineFunctionInfo();
			checkFunc = getcheckFunc(M, CheckMethod);
		}
		~Protection() {}
...
char GmemPass::ID = 0;
// 注册
static RegisterPass<GmemPass> X("gmem", "Gmem pass", false, false);
...
static RegisterStandardPasses GmemLoader_Ox(PassManagerBuilder::EP_ModuleOptimizerEarly, loadPass);
